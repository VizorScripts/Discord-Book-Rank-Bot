//
//          perfectly working with % stats and both pictures
//
...
function calculatePercentage(rank, categoryKey) {
    if (rank === 'N/A') return '';
    const numericRank = parseInt(rank.replace(/,/g, ''));
    const { total, displayTotal } = CATEGORY_METRICS[categoryKey];
...
function createEmbed() {
    return new EmbedBuilder()
        ...
        .addFields(
            { name: 'Overall Rank', value: `**#${currentRanks.overall}**${calculatePercentage(currentRanks.overall, 'overall')}`, inline: false },
            { name: 'Parenting (Disabilities)', value: `**#${currentRanks.parentingDisabilities}**${calculatePercentage(currentRanks.parentingDisabilities, 'parentingDisabilities')}`, inline: false },
            { name: 'Baby Guides', value: `**#${currentRanks.babyGuide}**${calculatePercentage(currentRanks.babyGuide, 'babyGuide')}`, inline: false },
            { name: 'General Birth', value: `**#${currentRanks.generalBirth}**${calculatePercentage(currentRanks.generalBirth, 'generalBirth')}`, inline: false }
        )
        .setFooter({ 
    ...
    try {
        await client.application.commands.create(
            new SlashCommandBuilder()
                .setName('rank')
                .setDescription('Show current Amazon rankings')
        );
